...
这将扫描您的项目及其所有内容，以了解该项目的内容并生成一个 `AGENTS.md` 文件。这有助于 opencode 更好地理解和导航项目。
...
对于从 Claude Code 迁移的用户，OpenCode 支持 Claude Code 的文件约定作为备选方案：
...
3. **Claude Code 文件**位于`~/.claude/CLAUDE.md`（除非被禁用）

每个类别中第一个匹配的文件被使用。例如，如果您同时拥有`AGENTS.md`和`CLAUDE.md`，则仅使用`AGENTS.md`。同样，`~/.config/opencode/AGENTS.md`优先于`~/.claude/CLAUDE.md`。
