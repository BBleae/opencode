...
规则通过模式匹配进行评估，**最后匹配的规则生效**。常见的模式是将通配的 `"*"` 规则放在前面，然后再放置更具体的规则。
...
使用 `external_directory` 允许工具调用启动 opencode 的工作目录之外的路径。这适用于任何采用路径作为输入的工具（例如 `read`、`edit`、`list`、`glob`、`grep` 和许多 `bash` 命令）。
...
这里允许的任何目录都会继承与当前工作空间默认相同的值。由于 [`read` 默认为 `allow`](#defaults)，`external_directory` 下的读取也是允许的，除非被覆盖。当需要在这些路径中控制工具时添加显式规则，例如在保留读取的同时阻止编辑：
...
- `read` — 读取文件（与文件路径匹配）
- `edit` — 所有文件修改（包括 `edit`、`write`、`patch`、`multiedit`）
- `glob` — 文件通配符（匹配通配符模式）
- `grep` — 内容搜索（匹配正则表达式模式）
...
如果您未指定任何内容，opencode 以宽松的默认权限启动：
...
`always` 批准的模式由该工具提供建议（例如，bash 批准通常将安全命令（如 `git status*`）列入白名单）。
