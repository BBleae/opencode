...
以下是与 opencode 配合良好的几个模型，排名不分先后。（这不是详尽的列表，也不一定是最新的）：
...
如果您配置了[自定义提供商](/docs/providers#custom)，则 `provider_id` 是配置中 `provider` 部分的键名，`model_id` 是 `provider.models` 中的键名。
...
内置提供商和模型名称可以在[Models.dev](https://models.dev) 上找到。
...
许多模型支持具有不同配置的多种变体。opencode 附带了流行模型的内置默认变体。
...
- `low` - 较低的 effort/Token 预算
- `high` - 较高的 effort/Token 预算
