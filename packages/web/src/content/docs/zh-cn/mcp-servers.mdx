...
:::tip
MCP 服务器会添加到您的上下文中，因此请谨慎选择启用哪些服务器。
:::
...
您还可以通过将`enabled`设置为`false`来禁用服务器。如果您想暂时禁用服务器而不将其从配置中删除，这非常有用。
...
要从组织的远程配置启用服务器，请使用配置 `enabled: true` 将其添加到本地配置：
...
使用`type`设置为`"local"`将本地 MCP 服务器添加到 MCP 对象中。
...
该命令是本地 MCP 服务器的启动方式。您还可以指定环境变量。
...
| `enabled`     | 布尔   |      | 在启动时启用或禁用 MCP 服务器。                                |
| `timeout`     | 数字   |      | 从MCP服务器获取工具的超时（以毫秒为单位）。默认为5000（5秒）。 |
...
`url`是远程 MCP 服务器的 URL，使用`headers`选项可以添加请求头。
...
| `oauth`   | 对象   |      | OAuth 身份验证配置。请参阅下面的 [OAuth](#oauth) 部分。        |
| `timeout` | 数字   |      | 从MCP服务器获取工具的超时（以毫秒为单位）。默认为5000（5秒）。 |
...
3. 安全地存储 Token 以供将来的请求

---

### 自动

对于大多数支持 OAuth 的 MCP 服务器，不需要特殊配置。只需配置远程服务器：
...
如果服务器需要身份验证，opencode 将在您第一次尝试使用它时提示您进行身份验证。如果没有，您可以使用 `opencode mcp auth <server-name>` [手动触发流程](#authenticating)。

---

### 预注册

如果您已有来自 MCP 服务器的客户端凭据，则可以配置它们：
...
`opencode mcp auth <server-name>` 命令将打开您的浏览器进行授权。授权后，opencode 将 Token 安全地存储在 `~/.local/share/opencode/mcp-auth.json` 中。

---

#### 禁用 OAuth

如果要禁用服务器的自动 OAuth（例如，对于使用 API 密钥的服务器），将`oauth`设置为`false`：
...
| `clientSecret` | 字符串          | OAuth 客户端密钥（如果授权服务器需要）。            |
| `scope`        | 字符串          | 授权期间请求的 OAuth 范围。                         |

#### 调试

如果远程 MCP 服务器无法进行身份验证，您可以通过以下方式诊断问题：
...
`mcp debug`命令显示当前身份验证状态、测试 HTTP 连接并尝试 OAuth 发现流程。
...
如果您有大量 MCP 服务器，您可以选择为每个代理启用它们并全局禁用它们。因此：

1. 全局禁用它作为工具。
2. 在您的[代理配置](/docs/agents#tools)中，启用 MCP 作为服务器工具。
...
:::note
MCP 服务器的工具以服务器名称作为前缀注册，要禁用服务器的所有工具，只需使用：
