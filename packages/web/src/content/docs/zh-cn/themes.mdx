...
使用 opencode，您可以从多个内置主题中进行选择，使用适合您的终端主题的主题，或定义您自己的自定义主题。
...
`system` 主题旨在自动适应您的终端方案。与使用固定颜色的传统主题不同，_system_ 主题：

- **生成灰度**：根据终端的背景颜色创建自定义灰度，确保最佳对比度。
- **使用 ANSI 颜色**：使用标准 ANSI 颜色 (0-15) 进行语法突出显示和 UI 元素，尊重终端的调色板。
- **保留默认设置**：使用 `none` 作为文本和背景颜色以保持原生的外观。
...
您可以通过使用 `/theme` 命令调出主题选择器来选择主题。或者您可以在 [config](/docs/config) 中指定它。
...
主题按以下顺序从多个目录加载，其中后面的目录覆盖前面的目录：

1. **内置主题** - 这些主题嵌入在二进制文件中
...
如果多个目录包含同名主题，则将使用优先级较高的目录中的主题。

---

### 创建主题

要创建自定义主题，请在主题目录中创建 JSON 文件。
...
特殊值 `"none"` 可用于任何颜色以继承默认的默认颜色。这对于创建与终端配色方案无缝融合的主题特别有用：

- `"text": "none"` - 使用终端的默认前景色
- `"background": "none"` - 使用终端的背景颜色
