...
OpenCode Zen 是 opencode 团队提供的经过测试和验证的模型列表。
...
您的 API 密钥。它是**完全可选的**，您不需要使用它即可使用
opencode。
...
市面上有很多模型，但其中只有少数几个
模型可以很好地用作编码代理。此外，大多数提供商的
配置非常不同；所以你会得到截然不同的性能和质量。

:::tip
我们测试了一组与 opencode 配合良好的模型和提供商。
:::

因此，如果您通过 OpenRouter 之类的服务使用模型，您永远无法
确定您是否获得了您想要的模型的最佳版本。
...
1. 我们测试了一组选定的模型，并与他们的团队讨论了如何
   最好地运行它们。
2. 然后我们与一些提供商合作以确保这些模型得到正确
   服务。
3. 最后，我们对 model/provider 的组合进行了基准测试，总结了
   一份我们觉得不错的推荐列表。

OpenCode Zen 是一个 AI 网关，可以让您访问这些模型。
...
1. 您登录 **<a href={console}>OpenCode Zen</a>**，添加您的账单
   详细信息，然后复制您的 API 密钥。
2. 您在 TUI 中执行 `/connect` 命令，选择 OpenCode Zen，然后粘贴 API 密钥。
3. 在 TUI 中执行 `/models` 以查看我们推荐的模型列表。

您需要按用量付费，并且您可以将余额添加到您的账户中。
...
使用 `opencode/<model-id>` 格式。例如，对于 GPT 5.2 Codex，您需要
在配置中使用 `opencode/gpt-5.2-codex`。
...
我们支持即用即付模式。以下是**每 100 万 Tokens 的价格**。
...
| Gemini 3 Pro（≤ 200K Tokens）       | 2.00 美元  | 12.00 美元 | 0.20 美元  | -          |
| Gemini 3 Pro（> 200K Tokens）       | 4.00 美元  | 18.00 美元 | 0.40 美元  | -          |
...
- GLM 4.7 Free 在 opencode 上限时提供。团队正在利用这段时间来收集反馈并改进模型。
- Kimi K2.5 Free 在 opencode 上限时免费提供。团队正在利用这段时间来收集反馈并改进模型。
- MiniMax M2.1 Free 在 opencode 上限时免费提供。团队正在利用这段时间来收集反馈并改进模型。
- Big Pickle 是一个隐形模型，在 opencode 上限时免费。团队正在利用这段时间来收集反馈并改进模型。

<a href={email}>如果您有任何疑问，请联系我们</a>。
...
例如，假设您将每月使用限额设置为 20 美元，Zen 将不会使用
一个月超过 20 美元。但如果您启用了自动充值，Zen 可能会
在您的余额低于 5 美元时向您收取超过 20 美元的费用。
...
- Big Pickle：在免费期间，收集的数据可用于改进模型。
- GLM 4.7 Free：在免费期间，收集的数据可用于改进模型。
- Kimi K2.5 Free：在免费期间，收集的数据可用于改进模型。
- MiniMax M2.1 Free：在免费期间，收集的数据可用于改进模型。
- OpenAI APIs：请求将根据 [OpenAI 的数据政策](https://platform.openai.com/docs/guides/your-data)保留 30 天。
- Anthropic APIs：请求将根据 [Anthropic 的数据政策](https://docs.anthropic.com/en/docs/claude-code/data-usage)保留 30 天。

---

## 对于团队

Zen 对团队也很有效。您可以邀请成员、分配角色、管理
您的团队使用的模型等等。
